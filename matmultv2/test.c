 ...
 *
*/
void sendworker(int *matrix1, int *matrix2, int count, int n);
...
int main(int argc, char *argv[]){
   ...
   if (numtasks % 9 != 0) {
      printf("Quiting. Number of MPI tasks must be 9.\n");
...
void master_node_process(char *argv[],int numtasks, int rank){
	...
	fclose(file2);
	int count=1;
	sendworker(sub1mat1,sub1mat2,count,n);
	//Load the 1-d arrays from the input files and Close the input files
	count++;
	sendworker(sub2mat1,sub3mat2,count,n);
	count++;
	sendworker(sub1mat1,sub2mat2,count,n);
	count++;
	sendworker(sub2mat1,sub4mat2,count,n);
	count++;
	sendworker(sub3mat1,sub1mat2,count,n);
	count++;
	sendworker(sub4mat1,sub3mat2,count,n);
	count++;
	sendworker(sub3mat1,sub2mat2,count,n);
	count++;
	sendworker(sub4mat1,sub4mat2,count,n);
...
void sendworker(int *matrix1, int *matrix2, int count, int n){
	int chunksize=n*n/4;
   MPI_Send(&n, 1, MPI_INT, count, TAG17, MPI_COMM_WORLD);
   MPI_Send(matrix1, chunksize, MPI_INT, count, (count*2)-1, MPI_COMM_WORLD);
   MPI_Send(matrix2, chunksize, MPI_INT, count, count*2, MPI_COMM_WORLD);
...
void worker_node_process(int numtasks, int rank){
   ...
   MPI_Recv(matrix1, chunksize, MPI_INT, 0, (rank*2)-1, MPI_COMM_WORLD, &status);
   MPI_Recv(matrix2, chunksize, MPI_INT, 0, rank*2, MPI_COMM_WORLD, &status);
	multiply(matrix1, matrix2, output, n/2);
   for(int i=0;i<chunksize;i++)
		printf("%d ",output[i]);
	MPI_Send(output, chunksize, MPI_INT, 0, TAG18, MPI_COMM_WORLD);
...
void multiply(int *matrix1,int *matrix2, int *output, int n){
	for(int i=0;i<n;i++){
		for(int j=0;j<n;j++){
			output[i*n+j]=0;
			for(int k=0;k<n;k++){
				printf("output[i*n+j]: %d",output[i*n+j]);
