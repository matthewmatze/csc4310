...
int main(int argc, char *argv[]){
   ...
   int sobel_y[9] = {1, 2, 1,
	...
	image = (struct pixel *)malloc(width*height*sizeof(struct pixel));

	output = (struct pixel *)malloc(width*height*sizeof(struct pixel ));
...
void readfile(struct pixel *image,int width,int height,FILE *file1){
   ...
   for(int i=0;i<width;i++){
      for(int j=0;j<height;j++){
         fread(&image[i*width+j].r, 1,1,file1);
         fread(&image[i*width+j].g, 1,1,file1);
         fread(&image[i*width+j].b, 1,1,file1);
      }
   }
}
void sobelx(struct pixel *image,struct pixel *output,int *sobel_x,int width,int height,int max){
   for(int row=1;row<width-1;row++){
      for(int col=1;col<height-1;col++){
         ...
         for(int i=0;i<3;i++){
            for(int j=0;j<3;j++){
               rgb[0]+=sobel_x[i*3+j]*(int)image[((row-1+i)*width)+(col-1+j)].r;
               rgb[1]+=sobel_x[i*3+j]*(int)image[((row-1+i)*width)+(col-1+j)].g;
               rgb[2]+=sobel_x[i*3+j]*(int)image[((row-1+i)*width)+(col-1+j)].b;
            }
         }
         for(int k=0;k<3;k++){
         ...
         output[row*width+col].r=(unsigned char)rgb[0];
         output[row*width+col].g=(unsigned char)rgb[1];
         output[row*width+col].b=(unsigned char)rgb[2];
      }
   }
...
void writefile(struct pixel *image,int width, int height, FILE *outfile,char *type, int max){
   ...
   for(int i=0;i<width;i++){
      for(int j=0;j<height;j++){
         fwrite(&image[i*width+j].r, 1,1,outfile);
         fwrite(&image[i*width+j].g, 1,1,outfile);
         fwrite(&image[i*width+j].b, 1,1,outfile);
      }
   }
}
